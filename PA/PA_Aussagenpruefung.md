...
| `sections/02_grundlagen.tex` | Masken werden mit demselben Mapping in die ideale Domäne gewarpt. | Matrixrunner und normaler Inline-Vollauf rufen `warp_masks_to_undistorted.py` nach COLMAP auf und übergeben `data/03_masks_ideal` an STS/SuGaR. Der Replaypfad ab `colmap` in `pipeline_lib.sh` besitzt den automatischen Warp weiterhin nicht. | BESTÄTIGT FÜR MATRIX-/INLINE-VOLLAUF, EINSCHRÄNKUNG BEI REPLAY | Replay ab `colmap` nur mit bereits idealem Maskensatz verwenden oder den Warp dort separat ergänzen. |
...
| `sections/03_konzept.tex` | Eine identische Bild-/Maskentransformation stellt in der Pipeline die Konsistenz her. | Für Matrixrunner und normalen `run_pipeline.sh`-Vollauf gilt nun: COLMAP → `warp_masks_to_undistorted.py` → idealer STS-Arbeitsbereich. Der Replaypfad ab `colmap` in `pipeline_lib.sh` setzt weiterhin einen passenden idealen Maskensatz voraus. | BESTÄTIGT MIT REPLAY-EINSCHRÄNKUNG | Replay ab `colmap` nicht mit Rohmasken starten; bei Bedarf dort denselben Warp ergänzen. |
