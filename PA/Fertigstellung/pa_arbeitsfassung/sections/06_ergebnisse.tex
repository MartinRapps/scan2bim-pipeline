...
Die archivierten Läufe verwenden durchgängig 240 Kameras. 210 dienen dem
Training und 30 der Evaluation auf einem festen, vom Training getrennten
Split (Auswahl und Begründung: Kapitel~\ref{sec:versuchsaufbau}). Jede
archivierte Kette erzeugte Route-A-Mesh,
Rendering, objektmaskierte Metriken, Centerline, B-Spline und GeoJSON. Der
Autopilotnachweis selbst ist über neun erfolgreiche archivierte Autopilot-Volläufe erbracht
...
SIFT-Limit von 4.096 Merkmalen pro Bild vereinbar. Eine höhere Pixelauflösung
...
vollständige Übersicht steht in Tabelle~\ref{tab:colmap-vorstudie} im Anhang~A. Guided
Matching erhöhte die Sparse-Punktzahl gegenüber Plain-SIFT 4.096 nur um rund
1,98\,\%, benötigte aber nahezu die doppelte Zeit und erhöhte den mittleren
Reprojektionsfehler. 8.192 Merkmale lieferten etwa 0,82\,\% mehr Punkte bei
5,5\,\% mehr Laufzeit.
...
Plain-SIFT 4.096 ohne Guided Matching: gleiche Registrierungsqualität bei
...
Eval-Split, Framezahl, Maskenabdeckung und Seeds übereinstimmen. Für die vorliegende Arbeit
...
von rund 2.011 Sekunden (720p), 1.650 Sekunden (qHD) und 1.199 Sekunden (low).
...
C & SuGaR-Coarse $c=9.000$, Diamond-Mesh-Z-Buffer & 79.711 & 156.935\\
D & SuGaR-Coarse $c=9.000$, Gaussian-Rasterizer-Tiefe & 68.618 & 135.522\\
\bottomrule
\end{tabularx}
...
begründet. Nachfolgende Abbildung~\ref{fig:pa-panel-sugar} ordnet die historischen
...
\begin{figure}[H]
  ...
  9.001 sowie bilaterale Vergleichsläufe im 9000er-/10.000er- und
...
LPIPS in 720p. Bei qHD liegen sie bei 21,69 dB, 0,761 und 0,141. Bei Low liegen sie bei 21,89 dB,
0,752 und 0,126. Für \texttt{OPENCV/SuGaR-Coarse} ergeben sich 21,45 dB,
0,764 und 0,153 in 720p. In qHD ergeben sich 21,62 dB, 0,764 und 0,137 sowie in Low 21,81 dB,
0,749 und 0,126. Tabelle~\ref{tab:sugar-followup-average} fasst die
...
die archivierte STS→Postprocess-Laufzeit. Grün und Lila kodieren STS
...
Beim paketierten Golden-Run-Lauf (\path{matrix_qualitaetsvergleich_20260818},
Arm 5~FPS/720p/\texttt{opencv\_a}; abgelegt unter \texttt{04\_Run-Archive/})
extrahierte DGtal 85 Rohpunkte. Der Grad-10-B-Spline erzeugte 372
Ausgabepunkte (\path{centerline_local_raw.csv} beziehungsweise
\path{centerline_local.csv}). Diese Zahlen belegen die
