...
Nachfolgende Abbildung~\ref{fig:pa-pipeline-flow} fasst diesen Datenfluss zusammen. Die
...
Genauigkeitsnachweis. Die Referenzierung ist zunächst rein lokal, das
...
\begin{tikzpicture}[
      ...
      \node[box, below right=1.55cm and 2.4cm of colmap] (gcp) {Originalmodell\\GCP-/CloudCompare-Zweig\\Matrix optional und in dieser Arbeit nicht validiert};
...
Die methodische Kernidee ist die Domänentrennung. Hierbei profitiert SfM von unmaskierten Hintergrundmerkmalen; GCP und SfM bleiben im
Originalraum, während STS und SuGaR in einer idealisierten, konsistenten
Bilddomäne arbeiten. Dadurch wird vermieden, dass eine Bildverzeichnung nur in den
...
Masken und übergibt \mbox{\path{data/03_masks_ideal}} an STS und SuGaR. Im
...
Der interaktive Vollauf führt Eingabeauswahl und Parameterentscheidungen durch.
...
bei low, jeweils inklusive SAM~3.1-, COLMAP-, Render- und Archivierungsanteil). Das Preset dient der bewussten Wahl zwischen Bildauflösung und Rechenaufwand.
...
(unabhängig von der Standardeingabe, stdin) nacheinander aus, das heißt er liest die Konfiguration aus
der TSV-Datei und berührt dabei nicht die Standardeingabe, die nachgelagerte
