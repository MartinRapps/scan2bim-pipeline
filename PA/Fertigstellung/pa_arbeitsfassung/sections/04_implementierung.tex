...
vereinheitlichen, wurde der SuGaR-Quellcode in einem lokalen Fork angepasst.
Die wesentlichen funktionalen Änderungen gegenüber Vanilla-SuGaR umfassen:

\begin{itemize}
  \item \textbf{Maskenbewusste Bildsupervision:} Einführung eines
        \texttt{SemanticMaskProvider} für maskierten RGB- und DSSIM-Loss
        ($\mathcal{L}_{\mathrm{RGB}}^{M}$, Gleichung~\ref{eq:masked-rgb-loss})
        in Coarse-Training und Refinement sowie maskierte
        Depth-Normal-Consistency;
  \item \textbf{Geometrieabsicherung:} Schutz vor zu kleinen Punktwolken und
        ungültigen Normalen bei der Poisson-Rekonstruktion sowie Korrektur
        der Coarse-Zählerprüfung für STS-Startzähler;
  \item \textbf{Extraktion und Evaluation:} Konfigurierbare Surface-Sampling-Parameter,
        Diagnoseoption für Tiefenkarten aus dem Gaussian-Rasterizer und
        Kardinalitätsprüfung für normalisierte feste Evaluations-Splits.
\end{itemize}

Der finale Stand ist als Git-Submodul unter Commit \texttt{a0fc37b} (Zweig
\texttt{project/masked-sugar}) eingebunden und wurde in allen archivierten
Matrix- und Folgeläufen verwendet. Das vollständige Diff ab der Vanilla-Basis
\texttt{48bbfdd} liegt als Datei \path{sugar_fork_diff_48bbfdd_a0fc37b.diff} im
digitalen Anlagenband (Ordner \texttt{05\_SuGaR-Fork/}). Eine chronologische
Zuordnung von Fehlerfällen und Korrekturschritten bietet Tabelle~\ref{tab:fehlerchronik}
im Anhang~D.
...
\path{04_sfm/undistorted/images/} & Ja & Durch
...
\path{03_masks/_attempts/} & Ja & Prompt-Fallbacks werden getrennt
...
sondern eine unveränderte Maske plus Erosionsstufen (ohne zusätzliche
Dilatation).\\
...
Arbeitsbereich entfernt werden. In allen Läufen dieser Arbeit war der erste
Prompt bereits erfolgreich, sodass die Fallback-Kette nie greifen musste. Sie
bleibt dennoch bewusst erhalten. Sie fängt leere Masken oder Speicherfehler
automatisch ab und dokumentiert in den Versuchsordnern nachvollziehbar, was
probiert wurde. Die STS-Kompatibilitätskopien können langfristig
...
$1280\times720$ Pixel, CRF 18, \texttt{OPENCV}, 4.096 SIFT-Merkmale,
Overlap 15, deaktiviertes Guided Matching, 7.000 STS-Iterationen und Route A.
...
alle Eingabefragen erhalten. Die Abfrage weist die gemessenen Gesamtlaufzeiten
der Produktionskonfiguration (Route A, 5 FPS,
\texttt{OPENCV}) aus. Rund 40 Minuten für 720p, 34 Minuten für qHD und
24 Minuten für low. Die Werte stammen aus dem archivierten
Verifikationsbatch \path{matrix_e2e_verifikation_260826}, dessen drei Läufe
die vollständige Kette inklusive SAM~3.1, COLMAP, Maskenwarp, Rendering und
Archivierung durchlaufen (Kopfphase 5--8 Minuten, STS--Postprocess
32/26/18 Minuten, Render- und Archivierungsschwanz etwa eine Minute). Ein
unabhängiger früherer Batch (\path{matrix_qualitaetsvergleich_20260818})
bestätigt die Werte mit 40:16, 34:35 und 24:02 Minuten. Die
Preset-Abfrage bleibt bewusst in beiden Modi
aktiv. Der Autopilot fragt nach seiner Aktivierung ausschließlich Lauf-Preset
und Objekt-Prompt ab und übernimmt alle übrigen Entscheidungen als
Standardeingaben. Die gewählte Auflösung wird im Run-Report protokolliert.
...
\begin{itemize}
  ...
  \item Matrixrunner, Inline-Vollauf und Replays mit frischem COLMAP-Lauf
        (Start ab \texttt{gcp}, \texttt{sam3} oder \texttt{colmap}) warpen die
        Rohmasken nach COLMAP in dieselbe ideale Bilddomäne wie die
        STS-Bilder. Die Ablagen bleiben dabei getrennt. Die idealen Bilder
        liegen unter \path{data/04_sfm/undistorted/images/}, die Rohmasken
        unter \path{data/03_masks/} und die gewarpten Masken unter
        \path{data/03_masks_ideal/}. Letztere werden an STS sowie SuGaR
        übergeben. Der Matrixrunner erzeugt zusätzlich einen festen Eval-Split
        aus den nichtleeren idealen Masken. Replays ab \texttt{sts},
        \texttt{sugar} oder \texttt{postprocess} erzeugen keine neue
        COLMAP-Szene und verwenden daher bereits bereitgestellte ideale
        Masken.
  \item Die SuGaR-Kameranamen werden als normalisierte Dateinamen ohne
        Endung (sogenannte Stems) verglichen. Leere
...
notwendigen Artefakte aus demselben Frame-, Kamera- und Iterationsstand
stammen. „Gültig" bedeutet hier: Die Punktwolke besitzt ausreichend viele
Stützpunkte, und die zugehörigen Normalen sind endliche, von null
verschiedene Vektoren, die sich normalisieren lassen. Trifft eine der
Prüfungen nicht zu, bricht der Lauf an dieser Stelle mit einem expliziten
Fehler ab, statt ein defektes Folgeprodukt still weiterzureichen. Der Lauf
wird im Archiv als fehlgeschlagen geführt. Der Matrixrunner archiviert jeden
Lauf, bevor der Live-Arbeitsbereich für die nächste Konfiguration bereinigt
wird.
...
DN-Consistency (laut Entwicklungsnotizen; nicht durch Laufmanifeste belegt);
da der DN-Term des Forks erst oberhalb des Zählerstands 9.000
aktiviert wird, blieb die Regularisierung so über die vollen 15.000
Iterationen rein dichtebasiert. Die lang gezogenen Strukturen entstanden dort
jedoch durch die falsche Übergabe und die fehlende Maskenbasiertheit
beziehungsweise die RGB-lastige Supervision, nicht durch die Route selbst und
nicht durch das Fehlen der DN-Consistency. Die Teilbilder 5 und 6
...
Fehlerbilder die heutigen Quality-Gates ausschließen. Die detaillierte
Chronologie dieser und weiterer Fehlversuche ist in
Tabelle~\ref{tab:fehlerchronik} im Anhang~D aufgeführt.
...
\begin{enumerate}
  \item \textbf{\path{metrics/sts_masked.json} (Original-STS / Route A):}\\
        Nach Abschluss des STS-Curriculums (7.000 Iterationen) wird der Standard-3DGS-
  ...
  \item \textbf{\path{metrics/sugar_refined_masked.json} (SuGaR-Refined):}\\
        ...
        \texttt{\{\string"status\string":\string"skipped\string",\string"reason\string":\string"no refined.ply exported\string"\}}.
\end{enumerate}
...
Schnittstellenregeln relevant. Die untenstehende Tabelle~\ref{tab:robustheit-kern} fasst die
