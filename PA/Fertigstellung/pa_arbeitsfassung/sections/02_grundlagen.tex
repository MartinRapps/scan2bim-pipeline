...
\path{ImageReader.single_camera=1} die Annahme eines gemeinsamen
...
Die beiden maskennutzenden Verlustpfade des Coarse-Trainings verwenden
unterschiedliche Maskenstufen. Die RGB-Supervision vergleicht das
Splat-Rendering jeder Trainingskamera pixelweise mit dem aufgenommenen Bild,
begrenzt auf die Maskenpixel (maskierter L1-Anteil plus maskierter
DSSIM-Anteil mit einem $11\times11$-Fenster; das lokale Fenster zieht am
Maskenrand begrenzt Kontext von außen ein). Sie verwendet die unveränderte
\texttt{default}-Maske, damit die Bildsupervision möglichst die volle
Objektprojektion abdeckt und Positionen, Opazitäten und Farben der
Objekt-Gaussians vollständig spezifiziert werden. Eine Farbprojektion auf
eine Oberfläche findet dabei nicht statt; die Farbübertragung aus den
Ansichten erfolgt erst in der nachgelagerten UV-Texturphase.

Die Depth-Normal-Consistency (DN) vergleicht dagegen die aus der gerenderten
Gaussian-Tiefe abgeleiteten Normalen mit den gerenderten Normalen und
supervidiert bewusst auf der erodierten \texttt{middle}-Stufe ohne Dilatation.
Der Grund ist die Silhouettenstabilität: Tiefe und daraus abgeleitete
Normalen sind direkt an Objektkanten unzuverlässig, und genau dort kann eine
...
Diese Zuordnung von \texttt{middle} zur DN-Stufe ist historisch bedingt.
Der DN-Term wird im projektinternen SuGaR-Fork erst oberhalb des
Zählerstands 9000 aktiviert; der aktuelle Coarse-Standard beträgt
\texttt{COARSE\_ITERATIONS=9000}, die aktuelle Matrix verwendet
ausschließlich die \texttt{original\_gs}-Route ohne Coarse-Training, und die
SuGaR-Folgeprüfung lief ebenfalls mit Zielzähler 9000
(vgl. Abschnitt~\ref{sec:versuchsaufbau}). In keinem der ausgewerteten Läufe
wurde der DN-Term daher tatsächlich ausgeführt — auch nicht im
\texttt{sugar\_coarse}-Modus. Die DN-Rolle von \texttt{middle} ist damit ein
historischer Entwicklungsstand ohne aktuell wirksamen Effekt auf die
Geometrie; aktuell wirksam bleibt \texttt{middle} allein in der
Coverage-Prüfung und der Kamera- beziehungsweise Eval-Split-Auswahl:
Ansichten mit leerer \texttt{middle}-Maske werden vom STS-Training
beziehungsweise von der Evaluation ausgeschlossen.
...
das Gaussian-bound Refinement und das UV-Baking. Sie verwendet
...
synthetische Ansichten für einen festen, beim Training ungesehenen Test-Kamerasplit
(\path{eval_frames.txt}) gerendert und pixelweise mit den echten Referenzbildern
verglichen.
...
Mathematisch und rendertechnisch sind beide Formate bei identischem Trainingsstand
vollständig äquivalent: Beide kapseln dieselben 3D-Gaussian-Parameter
($\boldsymbol{\mu}_i, \mathbf{S}_i, \mathbf{R}_i, \alpha_i$ und SH-Farbwerte) mit
voller 32-Bit-Gleitkommapräzision. Ein Laden der Tensoren aus einer \path{.pt}-Datei
in ein \path{SuGaR}-Modell liefert für dieselbe Kamera exakt dasselbe Pixelbild wie
eine hypothetisch daraus exportierte, standardkonforme \path{.ply}-Datei. Unterschiede
in den resultierenden Bildmetriken entstehen daher ausschließlich aus dem
unterschiedlichen Trainingszustand der Gaussians (z.\,B. 7000 Iterationen STS
gegenüber 9000 Iterationen SuGaR-Coarse-Optimierung), nicht aus dem Dateiformat.
