...
\midrule
\texttt{opencode} (CLI-Agent) & aktuelle Version, verschiedene LLMs je nach Aufgabe & Code-Erstellung, Refactoring, Pipeline-Debugging, Textüberarbeitung\\
...
Die Modellwahl innerhalb von \texttt{opencode} erfolgte je nach Aufgabenkomplexität und Kosten -- für einfache Korrekturen schlanke Modelle, für konzeptionelle oder fachliche Fragen leistungsfähigere Modelle. Alle KI-Ausgaben wurden manuell geprüft, verstanden und verantwortet.
...
\begin{itemize}
  \item \enquote{Die SAM-Masken enthalten mehrere Objekte. Passe die Logik so an, dass per Dilatation/Erosion automatisch genau ein zentrales Objekt übrig bleibt. Zeige nur die geänderte Funktion und erkläre die Parameter.}
  \item \enquote{Der Matrixrunner bricht ab, wenn die TSV-Datei per stdin konsumiert wird. Finde die Ursache und schlage eine stdin-unabhängige Schleife vor.}
  \item \enquote{Im SuGaR-Fork soll der RGB-Loss nur innerhalb der Objektmaske gewertet werden. Implementiere einen maskierten Loss $L_{\mathrm{RGB}}^{M}$ mit Normierung über die Maskenpixel und Schutz vor Division durch null.}
\end{itemize}
...
\textbf{Beispielhafte Prompts:}
\begin{itemize}
  \item \enquote{Ich habe PinHole und OpenCV getestet, beide liefern ähnliche Metriken. Welche Argumente sprechen fachlich für welchen Standard, und wie formuliere ich das ehrlich ohne Überinterpretation?}
  \item \enquote{Soll ich die Reproduzierbarkeit als eigenen Anhang oder im Methodenteil beschreiben? Nenne Vor- und Nachteile beider Varianten für diese Arbeit.}
...
\textbf{Beispielhafte Prompts:}
\begin{itemize}
  \item \enquote{Dieser Absatz über die Domänentrennung ist zu lang. Kürze ihn um 30\,\%, behalte aber COLMAP, Maskenwarp und ideale Domäne als Kernbegriffe.}
  ...
  \item \enquote{Die Tabelle tab:bildablagen ist zu breit. Schlage eine kompaktere Spaltenaufteilung vor, ohne Inhalt zu verlieren.}
\end{itemize}
...
\textbf{Worum es ging:} Erstellung der Panels (z.\,B.\ \texttt{pa\_panel\_sugar\_iteration}), Metrikplots (STS vs.\ SuGaR, Delta, Runtime) und deren Beschriftung.

\textbf{Beispielhafter Prompt:}
\begin{itemize}
  \item \enquote{Das Panel hat 9200er-Labels aus Dateinamen, soll aber konsistent 9001 zeigen. Schreibe ein Pillow-Skript, das die Kacheln neu zusammensetzt und nur 9001 ausgibt.}
\end{itemize}
