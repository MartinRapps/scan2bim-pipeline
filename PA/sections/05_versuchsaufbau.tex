...
Die Evaluation folgt einer gestuften Logik. Separate COLMAP-Voruntersuchungen
...
COLMAP-Standard & 4.096 Plain-SIFT, Overlap 15, Guided Matching aus,
Peak-Threshold 0,003\\
STS & 7.000 Gesamtiterationen, 5.000 Objektcurriculum\\
...
unter anderem 4.096 gegen 8.192 SIFT-Merkmale, Guided Matching und zwei
Kompressionsprofile. Alle fünf ausgewerteten Hauptläufe registrierten 240 von
240 Bildern. Die vollständige Tabelle mit Punktzahlen, Tracks,
Reprojektionsfehlern und Laufzeiten steht in
Tabelle~\ref{tab:colmap-vorstudie} im Anhang~A. Die Untersuchung begründet den Standard
4.096 Plain-SIFT und deaktiviertes Guided Matching, ist aber kein
vollständiger STS-/Meshvergleich.

\subsection{Autopilot und Produktionslauf}

Der kanonische Produktionslauf für den Testdatensatz verwendet das Video
\path{Alurohr_THWS.mp4} mit OPENCV, 5 FPS, 720p und Route A bis zu Poisson-Mesh,
B-Spline-Centerline und GeoJSON. Er ist als Autopilot-Vollauf archiviert.
Daneben existieren archivierte Autopilot-Volläufe für die übrigen
Auflösungsstufen (Abschnitt~\ref{sec:versuchsaufbau}, Erfolgskriterium~6).

\subsection{Kameramodell-/FPS-/Auflösungsmatrix}

Insgesamt umfasst die vorliegende Arbeit die drei nachfolgend aufgeführten Versuchsreihen. Die zugehörigen Ergebnisse
zeigen die Grafiken in Kapitel~7 (Ergebnisse) sowie die Detailtabellen in den
Anhängen:

\begin{itemize}
  \item die zusammengeführte historische 24er-Matrix aus den sechs
        Route-A-Versuchen von \path{matrix_full_pipe} (Kameramodell
        \texttt{SIMPLE\_RADIAL}) und 18
        Folgeexperimenten (\path{matrix_rest} mit den Kameramodellen
        \texttt{PINHOLE} und \texttt{OPENCV} als Route-A-Ablationen sowie der
        damaligen SuGaR-Variante mit \texttt{SIMPLE\_RADIAL}, jeweils über
        drei Auflösungen und zwei FPS-Profile);
  \item die Vierfeld-Ablation von Original-GS gegen SuGaR-Coarse mit zwei
        Tiefenrouten (Abschnitt unten sowie
        Abschnitt~\ref{sec:produktionskonfiguration});
  \item die abgeschlossene zwölfteilige SuGaR-Coarse-Folgematrix
        (\path{matrix_sugar_followup_12}).
\end{itemize}

Der Runner arbeitet seriell, weil alle Versuche denselben
Live-Workspace verwenden und zwischen den Läufen archiviert und bereinigt
wird.

\subsection{Vierfeld-Ablation der Meshgewinnung}
\label{subsec:vierfeld-ablation}
...
\begin{enumerate}
  ...
  \item C: SuGaR-Coarse $c=9.000$ mit Diamond-Mesh-Tiefe;
  \item D: SuGaR-Coarse $c=9.000$ mit Gaussian-Rasterizer-Tiefe.
\end{enumerate}
...
Die Auswertungsmethode der gerichteten Distanzen sowie ihre Grenzen sind in
Abschnitt~\ref{sec:produktionskonfiguration} beschrieben.

\subsection{Abgeschlossene SuGaR-Folgeprüfung}
\label{subsec:sugar-folgepruefung}
...
Tabelle~\ref{tab:fehlerchronik}. Da sämtliche Coarse-Läufe dieser Prüfung wie
auch die Coarse-Arme der Vierfeld-Ablation mit Zielzählerstand 9.000 trainiert
wurden, blieben die erst oberhalb dieses Zählerstands aktivierten DN-/SDF-
Terme des Forks in allen ausgewerteten Läufen ohne Wirkung.
...
Der feste Eval-Split teilt die 240 registrierten Kameras eines Laufs in 210
Trainings- und 30 Evaluationsansichten. Die Evaluationsansichten sind genau
deshalb aussagekräftig, weil ihre Bilder nicht am Training beteiligt waren.
Die Aufteilung ist als Liste archiviert und in allen Läufen identisch, sodass
die Metriken zwischen den Konfigurationen vergleichbar bleiben.
...
Die vorliegende Arbeit gilt als technisch erfolgreich, wenn:
...
\begin{enumerate}
  ...
  \item die Ergebnisse für eine spätere GNSS-basierte Bachelorbewertung
    ...
    \item interaktiver Vollauf, Autopilot und Matrixrunner in ihrer
      jeweiligen Funktion eindeutig dokumentiert und über archivierte Läufe
      nachgewiesen sind. Dies sind neun erfolgreiche Autopilot-Volläufe (720p, qHD und
      low), der kanonische Produktionslauf sowie die Matrix- und
      Folgematrixbatches.
\end{enumerate}
