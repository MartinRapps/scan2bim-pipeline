...
konservativeren Stufen entfernen unsichere Randpixel, können bei sehr dünnen
Objekten aber auch relevante Objektfläche verlieren. Deshalb werden
Maskenstufe und Dilatation als explizite Parameter behandelt.

Ein Einzelbild- oder SAHI-Ansatz könnte kleine Strukturen räumlich vergrößert
verarbeiten. Er besitzt jedoch nicht automatisch dieselbe zeitliche
Objektkontinuität. Da ein systematischer Vergleich dieser Frontends nicht
ausgeführt wurde, bleibt er außerhalb der PA und wird nur als möglicher
Folgeversuch genannt.
...
\parencite{schoenberger2016sfm}. Das Projekt verwendet unmaskierte Bilder:
...
Die Matrix untersucht drei Kameramodelle. \texttt{PINHOLE} modelliert getrennte
Brennweiten und einen Hauptpunkt, aber keine Verzeichnung. Es setzt damit eine
...
zusätzliche Parameter schlecht konditionieren können.
...
\parencite{kerbl2023gaussian}. Ein Gaussian $i$ besitzt unter anderem einen
...
\parencite{lu2025segment}. In der Projektpipeline ersetzen die extern erzeugten
SAM-3.1-Masken das ursprünglich vorgesehene Segmentierungsfrontend. STS
initialisiert die Szene aus dem COLMAP-Modell, ordnet Gaussians den Objekten zu
und optimiert sie in einer Objekt-/Maskenphase sowie einer anschließenden
All-Object-Phase. Der Projektstandard umfasst 7000 Gesamtiterationen, davon
5000 innerhalb des Objektcurriculums.

\subsection{Objektselektion und Meshgewinnung}

Die Ziel-Gaussians werden über die konfigurierte Objekt-ID selektiert. Ein
Opazitätsfilter kann schwach sichtbare Primitive entfernen. Die in der PLY
gespeicherte Logit-Opazität $o_i$ wird dafür mit
...
in den Bereich $[0,1]$ überführt. Nach Objekt-, Opazitäts- und optionaler
Farbauswahl erzeugt die Pipeline eine getrennte Hochopazitätskopie. Das Setzen
auf $\alpha=0{,}999999$ ist eine Initialisierungspolitik für die geometrische
Extraktion, keine physikalische Aussage über Materialtransparenz.
...
Oberflächenstichproben ein Poisson-Mesh
\parencite{guedon2024sugar,kazhdan2006poisson}. Der lokale Fork kann den
...
wobei $M$ die ausgewählte Objektmaske bezeichnet. Der Ausdruck
...
Original-STS-Gaussians, projizierte Diamond-Mesh-Tiefe, Surface-Sampling,
Poisson-Rekonstruktion, Dichtebereinigung, Decimation und optionale
Vertexprojektion. Die vollständige SuGaR-Coarse-Route bleibt als
...
zusätzliche Coarse-Optimierung zuvor die Gaussian-Geometrie?

\subsection{Centerline und B-Spline}

DGtal voxeliert das Mesh und führt eine topologieerhaltende Skelettierung aus
\parencite{dgtalProject}. Auf verrauschten, dünnwandigen Oberflächen können
dabei Medialflächen und zahlreiche kurze Äste entstehen. Der produktive
...
Der Grad ist ein algorithmischer Glättungsparameter; er begründet keine
metrische Genauigkeit. Die Anzahl der ausgegebenen Samples verändert nur die
