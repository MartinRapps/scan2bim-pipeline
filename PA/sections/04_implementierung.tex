...
die vollständige Kette inklusive SAM3, COLMAP, Maskenwarp, Rendering und
...
\begin{itemize}
  ...
  \item Matrixrunner und Inline-Vollauf warpen die Rohmasken nach COLMAP in
        ...
        nichtleeren idealen Masken. Replays ab \texttt{sts},
        \texttt{sugar} oder \texttt{postprocess} verwenden bereits
        bereitgestellte ideale Masken; ein Replay ab \texttt{colmap} führt
        denselben Maskenwarp selbst aus, sodass auch das Diagnoseprofil
        \texttt{colmap-stop} einen fortsetzbaren, domänenkonsistenten
        Zwischenstand hinterlässt.
...
\begin{enumerate}
  \item \textbf{\path{metrics/sts_masked.json} (Original-STS / Route A):}\\
        ...
        Skript \path{render_standard_splat} rendert die ungesehenen Test-Kameras;
        anschließend berechnet \path{evaluate_masked_splat_metrics.py} die
        objektmaskierten Werte für PSNR, SSIM und LPIPS gegen die Ground-Truth-Bilder.
