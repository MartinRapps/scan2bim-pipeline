...
nur die archivierten STS-bis-Postprocess-Zeiten. SAM3, COLMAP, Rendering und
Metrikcontainer sind darin nicht vollstaendig enthalten. Fuer die Hauptaussage
...
Die kombinierte Uebersicht aus dem historischen \texttt{matrix\_full\_pipe}-
Batch und dem Folge-Batch \texttt{matrix\_rest} ist in
Abbildung~\ref{fig:matrix-overview} dargestellt. Die Zeilen beschreiben die
jeweilige Laufvariante und Aufloesung. Innerhalb jeder FPS-Gruppe werden PSNR,
SSIM und LPIPS ausschliesslich innerhalb der Objektmaske angegeben. Die
Average-Spalte ist nur dann belegt, wenn sowohl der 2-FPS- als auch der
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_overview_table.pdf}
    \caption{Vorlaeufige quantitative Matrixauswertung der objektmaskierten
    STS-7000-Metriken. Die Gruppen zeigen 2 FPS, 5 FPS und den Average aus
    beiden Profilen; gruen markiert das beste verfuegbare Ergebnis innerhalb
    der jeweiligen Spaltengruppe. Fehlgeschlagene Routen werden farblich als
    unvollstaendig ausgewiesen.}
    \label{fig:matrix-overview}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_combined_overview_table.pdf}
    \caption{Kombinierte Overview der erfolgreich abgeschlossenen historischen
    STS-/Original-GS-A-Routen und der SuGaR-Coarse-Folgematrix. Die
    Modellstufen werden getrennt ausgewiesen; historische Fehlrouten sind nicht
    als erfolgreiche SuGaR-Metriken enthalten.}
    \label{fig:matrix-combined-overview}
\end{figure}

Die Grafik ist als Screening- und Effizienzdarstellung geeignet, ersetzt aber
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_sugar_overview_table.pdf}
    \caption{Abgeschlossene SuGaR-Coarse-Folgematrix mit objektmaskierten
    PSNR-, SSIM- und LPIPS-Werten für SIMPLE-RADIAL und OPENCV. Average wird
    nur aus vollständigen 2-/5-FPS-Paaren gebildet.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_sugar_status_table.pdf}
    \caption{Statusübersicht der zwölf abgeschlossenen SuGaR-Coarse-Folgeläufe.}
\end{figure}

Die zugehörigen Einzelgrafiken für PSNR, SSIM, LPIPS und das sekundäre
Zeit-/Qualitätsverhältnis liegen unter
	exttt{docs/grafiken/matrix\_sugar\_psnr\_table.pdf},
	exttt{matrix\_sugar\_ssim\_table.pdf},
	exttt{matrix\_sugar\_lpips\_table.pdf} und
	exttt{matrix\_sugar\_time\_quality\_table.pdf}. Die gemeinsame CSV-Datei
	exttt{matrix\_thesis\_data.csv} kennzeichnet die Modellstufe über
	exttt{model\_stage}; STS-Baselines und SuGaR-Coarse-Metriken werden damit
nicht vermischt.
