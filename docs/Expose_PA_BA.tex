...
*   **Anwendervorteil:** Im Autopilot ist kein Benutzereingriff für die
Pipelinefortsetzung erforderlich; der GCP-Breakpoint wird dabei bewusst
übersprungen und ein Fallback verwendet. Für eine echte GCP-Registrierung
stehen die Bildmarkierungs-UI und alternativ der CloudCompare-Breakpoint
zur Verfügung. Nur nach erfolgreicher Registrierung werden die
georeferenzierten GIS-Daten mit der erzeugten Matrix exportiert.
...
\begin{itemize}
    ...
    \item \textbf{Container B (SfM):} Ein offizielles NVIDIA-Image für COLMAP (CUDA 11.8 oder 12.1). Der interaktive CLI-Pfad kann nach diesem Container für das manuelle GCP-Point-Picking in CloudCompare pausieren. Der bevorzugte aktuelle Bildworkflow markiert die GCPs dagegen in der UI und berechnet Matrix und Report anschließend im Post-Processing-Container.
...
\begin{itemize}
    \item \textbf{Datensatz und Ziel:} Das Alurohr ist der aktuelle lineare
        ...
        \item \textbf{Ausfuehrung und SuGaR-Versionierung:} Der Produktionsweg ist
            ...
            \texttt{4568304} aktualisiert den Gitlink auf
            diesen Stand.
    ...
    \item \textbf{Frame- und COLMAP-Standard:} Der aktuelle Default ist
        ...
        \item \textbf{Segmentiertes Gaussian-Mesh und SuGaR-Vergleichsroute:} STS
            verwendet 7000 Gesamtiterationen, davon 5000 Iterationen fuer die
            Objekt-/Maskenphase und 2000 fuer die anschliessende All-Object-Phase.
            Der aktuelle Produktionsstandard ist jetzt
            \texttt{SUGAR\_MESH\_MODE=original\_gs} (Variante A): Die vorbereitete
            Hochopazitaetskopie der STS-Gaussians wird direkt als geometrische
            Zielrepraesentation verwendet. Die Meshgewinnung nutzt die projizierte
            Diamond-Mesh-Tiefe sowie Surface-Sampling, Poisson, Dichtebereinigung,
            Decimation und Vertexprojektion; SuGaR-Coarse-Optimierung,
            Gaussian-bound Refinement und UV-Baking werden uebersprungen. Der
            mask-aware SuGaR-Pfad bleibt mit
            \texttt{SUGAR\_MESH\_MODE=sugar\_coarse} als Vergleichsroute erhalten
            und verwendet standardmaessig \texttt{dn\_consistency} mit dem
            Coarse-Zielzaehler $c=9000$, \texttt{MASK\_LEVEL=default},
          ...
          Refinement und keinen Consensus-Crop. Bei dieser Legacy-Route werden die im
          ...
          spaetere Erweiterung. Fuer Variante A wird das Coarse-PLY unter
          \texttt{data/06\_mesh/<export-name>/coarse.ply} erhalten und als
          untexturiertes \texttt{refined.obj} fuer den bestehenden
          Centerline-Einstieg konvertiert; der Dateiname bezeichnet dabei keine
          SuGaR-Refinement-Stufe. Die Laufzeitausgaben werden unter
...
\subsection*{GCP/UI-Korrektur und verifizierter Iststand}
Die technische GCP-Ansicht wurde nach einem reproduzierten Fehlerszenario
nochmals gegen den aktuellen Arbeitsbaum und den vorgesehenen Post-Processing-
Container geprüft. Ausgangsseitig erschien die Frame-Liste zunächst leer,
wenn die UI vor dem COLMAP-Lauf geöffnet wurde. Nach COLMAP wurden die Daten
jedoch nicht automatisch neu geladen und es wurde kein erster Frame geöffnet.
Zusätzlich wurden verschachtelte Dateien aus der rekursiven Serverantwort im
Dashboard nicht angezeigt. Diese Befunde betrafen die Bedienoberfläche und
waren kein Nachweis eines Fehlers in der SfM-Geometrie.

Die Registrierung schlug zunächst aus zwei voneinander unabhängigen
technischen Gründen fehl. Erstens wurde eine Standardzeile aus COLMAPs
\texttt{cameras.txt} fälschlich so interpretiert, als müsse vor den
Kameraparametern eine Parameteranzahl stehen. Das Standardformat enthält diese
Anzahl nicht; bei der aktuellen Kamera
\texttt{SIMPLE\_RADIAL} folgen direkt vier Parameter. Zweitens übergab der
UI-Server beim Docker-Aufruf Host-Pfade an den Container. Dort sind die
Bind-Mounts jedoch unter \texttt{/data} verfügbar. Auf dem verwendeten System
führte außerdem die Compose-Standard-UID $1000:1000$ zu fehlenden
Schreibrechten für \texttt{matrix.txt}.

Die Korrektur umfasst daher vier Teile:
\begin{itemize}[noitemsep]
    \item Der GCP-Parser akzeptiert das Standardformat und das ältere
        projektinterne Format mit expliziter Parameteranzahl und prüft
        Modell, Bildgröße und Parameterzahl.
    \item Die UI bietet ``Frames aktualisieren'', öffnet nach erfolgreichem
        Laden automatisch den ersten registrierten Frame und zeigt Quelle,
        fehlende Bilddateien sowie strukturierte Docker-/Native-Fehler an.
    \item Der Docker-Aufruf verwendet die Containerpfade unter
        \texttt{/data/04\_sfm} und \texttt{/data/01\_raw} sowie die aktuelle
        Host-UID/GID. Dadurch können Matrix und Report in den Bind-Mount
        geschrieben werden.
    \item Die Dateivorschau flacht rekursive \texttt{children}-Einträge auf
        Blattdateien ab, zeigt deren relativen Pfad und begrenzt lediglich
        die sichtbare Liste bei sehr großen Ordnern.
\end{itemize}

Der verifizierte Stand verwendet 240 registrierte COLMAP-Frames und 17
Bildbeobachtungen für drei lokale/synthetische GCPs (5, 5 und 7 Beobachtungen).
Die Berechnung über den UI-Endpunkt lief im Docker-Modus erfolgreich und
schrieb \texttt{data/04\_sfm/matrix.txt} sowie
\texttt{data/04\_sfm/gcp\_report.json}. Der Report weist ein Gesamt-RMSE von
$0{,}05933\,\mathrm{m}$ und ein maximales GCP-Residuum von
$0{,}07362\,\mathrm{m}$ aus. Die Reprojektions-RMSE betragen für die drei
GCPs $3{,}7916\,\mathrm{px}$, $2{,}8042\,\mathrm{px}$ und
$6{,}9943\,\mathrm{px}$. Drei nichtkollineare GCPs sind das mathematische
Minimum der 3D-Ähnlichkeitstransformation; für eine robuste praktische
Kontrolle werden mindestens vier räumlich gut verteilte GCPs empfohlen.

Die Regressionstests in \texttt{tests/test\_gcp\_register.py} umfassen elf
erfolgreiche Tests. Ein Browser-Smoke-Test bestätigte den automatischen Start
bei \texttt{00000.jpg}, 240 sichtbare Vorschaubilder, die Darstellung des
GCP-Reports und die Anzeige verschachtelter Dateipfade. Damit ist die
technische Bedien- und Berechnungskette repariert. Die Zahlen sind wegen der
lokalen/synthetischen GCP-Koordinaten jedoch kein geodätischer
Genauigkeitsnachweis für eine reale Trasse. Ebenso beweist ein erfolgreicher
Autopilot-Status ohne erzeugte \texttt{matrix.txt} weiterhin keine echte
GCP-Registrierung; dies muss am Matrix- und Reportartefakt geprüft werden.

\subsection*{Reproduzierbarer Smoke-Test und Replay eines Matrixlaufs}
Der Lauf \texttt{matrix\_smoke\_low\_pipe\_full} ist kein vollstaendiger
Matrixlauf und kein wissenschaftliches Endergebnis. Der Begriff
\emph{Smoke-Test} bezeichnet hier einen begrenzten Integrations- und
Vertragstest: Es wird ein einzelner, ressourcenschonender Versuch durch die
gesamte bisherige Verarbeitungskette gefuehrt, um die Uebergaben zwischen SAM3,
COLMAP, STS und SuGaR zu pruefen, bevor die teure Parameter-Matrix gestartet
wird. \texttt{full} bedeutet in dieser Run-ID, dass der einzelne Versuch nicht
im Masken-only-Modus abgebrochen wurde; es bedeutet nicht, dass alle 24
Matrixkombinationen ausgefuehrt wurden.

Der reproduzierbare Testaufbau besteht aus:
\begin{itemize}[noitemsep]
    \item Prompt \texttt{pipe}, 5 FPS und 640$\times$360 Pixeln (\texttt{low});
    \item \texttt{SIMPLE\_RADIAL} auf Rohframes, anschliessende ideale
        PINHOLE-Szene und geometrisch gewarpte Masken;
    \item Variante A mit \texttt{SUGAR\_MESH\_MODE=original\_gs}, also dem
        direkten hochopaken STS-Gaussian-Eingang ohne SuGaR-Coarse-Training;
    \item 240 Bilder, fester Split mit jedem achten Bild als Testbild,
        somit 210 Trainings- und 30 Testkameras;
    \item Maskenqualitaetsgrenze von 30 Prozent leeren oder fehlenden
        \texttt{middle}-Masken;
    \item 7000 STS-Iterationen, 200000 Mesh-Vertices, 5000000
        Oberflaechenstichproben und reproduzierbarer Seed 42.
\end{itemize}

Beim ersten vollstaendigen Low-Lauf trat in SuGaR vor der Mesh-Extraktion
folgender Fehler auf:
\begin{quote}
\texttt{IndexError: list index out of range} in
\texttt{CamerasWrapper(self.test\_cam\_list)}.
\end{quote}
Die Ursache war kein leerer SAM3-Maskensatz. Die Maskenfilterung behielt
240 von 240 Kameras. Der Fehler entstand beim festen Evaluationssplit: SuGaRs
\texttt{cameras.json} speicherte Bildnamen als Stems wie \texttt{00000}, die
gemeinsame \texttt{eval\_frames.txt} enthielt dagegen \texttt{00000.jpg}.
Die alte exakte Zeichenkettenpruefung fand dadurch keine Testkamera und
uebergab \texttt{CamerasWrapper([])}.

Die Loesung normalisiert nun Basename und Dateiendung in
\texttt{sugar\_scene/gs\_model.py} sowie im Standard-Renderer. Falls ein
Basename-Vergleich nicht ausreicht, wird die abschliessende numerische
Frame-ID verglichen, beispielsweise \texttt{frame\_00008} mit
\texttt{00008.jpg}. Fehlende oder leere Split-Dateien sowie leere Kamera-
listen werden explizit mit einer aussagekraeftigen Fehlermeldung abgebrochen.
Jeder SuGaR-Lauf protokolliert ausserdem die Anzahl der Split-Eintraege, der
geladenen Kameras und der erfolgreichen Matches. Die Datei wird ueber
\texttt{SUGAR\_EVAL\_FRAMES\_PATH=/data/05\_3dgs/eval\_frames.txt} an den
SuGaR-Container weitergegeben.

Da ein Replay an einer Schrittgrenze kein Fortsetzen innerhalb eines
abgebrochenen Trainingsprozesses ist, muessen die Eingabeartefakte des
Startpunkts wieder im Live-Arbeitsbereich liegen. Fuer den SuGaR-Replay und
die nachfolgenden Render-/Metrikschritte sind dies insbesondere die idealen
Masken unter \texttt{data/03\_masks}, die idealen COLMAP-Bilder unter
\texttt{data/04\_sfm/undistorted/images}, Sparse-Metadaten,
\texttt{cameras.json}, der vollstaendige sowie der hochopake STS-Ply bei
Iteration 7000 und die gemeinsame \texttt{eval\_frames.txt}. Das
Wiederherstellungsskript
\texttt{tools/restore\_matrix\_replay.sh} kopiert nur diese Artefakte aus dem
Archiv und legt den benoetigten \texttt{data/05\_3dgs/images}-Link an; Rohvideo,
Hugging-Face-Cache und das Archiv werden nicht veraendert.

Der anschliessende Test mit \texttt{run\_pipeline.sh --from sugar}, Variante A,
\texttt{STOP\_AFTER\_COARSE\_MESH=1}, 200000 Vertices und 5000000 Samples war
erfolgreich. SuGaR meldete 30 passende Evaluationskameras und 210
Trainingsbilder; die Original-GS-Meshroute erzeugte einen gueltigen Coarse-Ply
und das kompatible \texttt{refined.obj}. SuGaR-Coarse-Training, Refinement,
UV-Baking und Postprocessing wurden in diesem ersten Mesh-Smoke-Aufruf bewusst
nicht ausgefuehrt.

Anschliessend wurden Postprocessing und Bildmetriken separat auf denselben
Artefakten ausgefuehrt. DGtal extrahierte erfolgreich eine Centerline mit 87
Rohpunkten; der Grad-10-B-Spline erzeugte 380 Punkte. Das lokale GeoJSON sowie
die translation-only-Fallback-Ausgaben wurden geschrieben. Eine echte
4x4-Georeferenzierung war in diesem Archivlauf nicht moeglich, weil
\texttt{data/04\_sfm/matrix.txt} fehlte; deshalb ist die Fallback-Geometrie
kein Genauigkeitsnachweis.

Der bestehende STS-7000-Splat wurde anschliessend auf den 30 festen Testbildern
gerendert. Die ausschliesslich objektmaskierten Werte betragen:
\begin{center}
\begin{tabular}{l r}
\textbf{Metrik} & \textbf{Wert}\\
\hline
PSNR$_{masked}$ & 29.8366\,dB\\
SSIM$_{masked}$ & 0.929862\\
LPIPS$_{masked}$ & 0.082681\\
Evaluationsbilder & 30\\
\end{tabular}
\end{center}
Full-frame-Metriken wurden nicht berechnet. Variante A erzeugt keinen neuen
SuGaR-Splat; der STS-7000-Splat ist daher zugleich die passende Gaussian-
Baseline fuer A. Die Ergebnisse belegen die technische Funktionsfaehigkeit von
Rendern, Maskierung, Metrikberechnung und Postprocessing auf diesem
Smoke-Test, sind aber weder ein Vergleich aller 24 Matrixlaeufe noch ein
Nachweis der spaeteren geometrischen Genauigkeit.

Der erste anschliessende Start der vollstaendigen Matrix unter
\texttt{MATRIX\_BATCH\_ID=matrix\_full\_pipe} erzeugte nur sechs statt 24
Versuche. Erfolgreich waren die vier \texttt{simple\_radial\_a}-Laeufe bei
5~FPS in 720p, QHD und Low sowie bei 2~FPS in 720p. Die beiden 2-FPS-Laeufe in
QHD und Low erreichten SuGaR, Postprocessing und Rendering, brachen jedoch bei
der Metrikberechnung am leeren Eval-Frame \texttt{00056.jpg} ab. Die
30-Prozent-Maskengrenze war mit 2 von 96 leeren Masken bewusst nicht erreicht;
der Metrikcode war an dieser Stelle strenger als der vorgelagerte
Qualitaetsguard.

Die Varianten \texttt{simple\_radial\_sugar}, \texttt{pinhole\_a} und
\texttt{opencv\_a} wurden in diesem Batch nicht fachlich getestet. Eine
Bash-Schleifenimplementierung verbrauchte durch vererbten Standardinput nach
dem ersten Varianteneintrag die restlichen TSV-Zeilen. Der Matrix-Runner wurde
danach auf stdin-unabhaengige \texttt{for}-Schleifen umgestellt. Zusaetzlich
wird der feste Eval-Split nun erst nach dem Mask-Warping aus nichtleeren idealen
Masken erzeugt; ein teilweise nicht gematchter Split wird in SuGaR explizit
abgelehnt. Der historische Batch bleibt als reproduzierbarer Fehlernachweis
archiviert.

\subsubsection*{Laufunterschiede und Hypothese zur 2-FPS-Maskenleere}
Die bisherigen Matrixlaeufe unterscheiden zwei voneinander getrennte
Einflussfaktoren: die zeitliche Samplingdichte und die raeumliche
Bildaufloesung. Das 5-FPS-Profil erzeugte bei der rund 48-sekuendigen Aufnahme
240 Bilder, das 2-FPS-Profil dagegen 96 Bilder. Die Aufloesungen wurden mit
maximaler Bildseite 1280, 960 beziehungsweise 640 Pixeln erzeugt. Die
folgende Tabelle fasst den ersten Batch zusammen; die Werte beziehen sich auf
die damalige feste Evaluationsliste und ausschliesslich objektmaskierte
STS-Metriken.

\begin{center}
\small
\begin{tabularx}{\textwidth}{l l r l X}
\textbf{FPS} & \textbf{Aufloesung} & \textbf{Frames} &
\textbf{Maskenstatus} & \textbf{Ergebnis}\\
\hline
5 & 720p & 240 & 0/240 leer & erfolgreich; PSNR 29.4164, SSIM 0.894742, LPIPS 0.123718\\
5 & QHD & 240 & 2/240 leer (0.83\,\%) & erfolgreich; PSNR 28.2992, SSIM 0.884874, LPIPS 0.119272\\
5 & Low & 240 & 0/240 leer & erfolgreich; PSNR 29.7890, SSIM 0.929571, LPIPS 0.082164\\
2 & 720p & 96 & 0/96 leer & erfolgreich; PSNR 26.3382, SSIM 0.831956, LPIPS 0.121868\\
2 & QHD & 96 & 2/96 leer (2.08\,\%) & Mesh/Postprocess/Rendern erfolgreich; Metrikabbruch bei \texttt{00056.jpg}\\
2 & Low & 96 & 2/96 leer (2.08\,\%) & Mesh/Postprocess/Rendern erfolgreich; Metrikabbruch bei \texttt{00056.jpg}\\
\end{tabularx}
\end{center}

Die Vermutung des Projekts ist, dass die leeren Masken bei 2 FPS durch eine
Kombination aus groesserem zeitlichem Bildsprung und geringerer Objekt-
ueberdeckung entstehen. Der SAM-3-Video-Predictor kann bei dichterer zeitlicher
Abtastung die Objektmaske von einer Ansicht in die naechste mit kleineren
Bewegungen propagieren. Bei 2 FPS ist die Bewegung zwischen zwei Eingabebildern
groesser; bei QHD und Low stehen fuer das duenne Rohr zusaetzlich weniger
stabile Pixel und Konturen zur Verfuegung. Dadurch kann die Propagation in
einzelnen Frames abbrechen oder eine leere Maske erzeugen.

Diese Erklaerung ist derzeit eine plausible Arbeitshypothese und noch kein
isolierter Kausalnachweis. Gegen eine einfache Aussage ``niedrige Aufloesung
allein verursacht leere Masken'' sprechen die 0/240 leeren Masken im 5-FPS-
Low-Lauf und die 2/240 leeren Masken im 5-FPS-QHD-Lauf. Ebenso blieb der
2-FPS-720p-Lauf leer-maskenfrei. Die Beobachtung spricht daher eher fuer eine
Interaktion von FPS, Aufloesung, konkreter Kamerabewegung und
frameabhaengiger SAM3-Stabilitaet. Die allgemeine 30-Prozent-Schranke wurde in
keinem dieser beiden 2-FPS-Fehlerfaelle erreicht; der Abbruch entstand nur,
weil der alte feste Eval-Split den konkreten leeren Frame \texttt{00056.jpg}
auswaehlte. Der aktuelle Runner erstellt den Eval-Split deshalb erst nach dem
Warping und beruecksichtigt nur nichtleere ideale \texttt{middle}-Masken.

Die bereits ausfuehrlich getestete Produktionsroute
\texttt{simple\_radial\_a} wird fuer die naechste Matrix nicht erneut
gerechnet. Sie wurde aus \texttt{tools/experiment\_matrix.tsv} entfernt; die
historischen Ergebnisse und Archive bleiben erhalten. Die verbleibende
Matrix umfasst damit drei Varianten
(\texttt{simple\_radial\_sugar}, \texttt{pinhole\_a}, \texttt{opencv\_a}),
drei Aufloesungen und zwei FPS-Profile, also 18 neue Versuche.

Die in den fehlgeschlagenen \texttt{simple\_radial\_sugar}-Versuchen
vorhandenen \texttt{sts\_masked.json}-Dateien sind dabei ausschliesslich
STS-Baseline-Metriken. Der Matrix-Runner berechnet diese vor dem separaten
SuGaR-Coarse-Rendering. Die SuGaR-Metrikdatei wird erst im nachfolgenden
Schritt geschrieben; dieser brach wegen des fehlenden Python-Importpfads fuer
\texttt{sugar\_scene} ab. Die Baseline-Metrik darf daher nicht als
erfolgreiches SuGaR-Ergebnis interpretiert werden.

Die Zeit-/Qualitaetsmatrix wird als sekundäre Engineering-Auswertung
beibehalten, nicht als alleinige wissenschaftliche Rangliste. Niedrigere
Aufloesungen koennen durch Downsampling als scheinbar bessere Bildmetriken
erscheinen: Kantenversatz, hochfrequentes Rauschen und kleine Maskenfehler
werden bereits bei der Bildbildung geglaettet. Das ist keine automatische
Aussage ueber bessere 3D-Geometrie. Die Messwerte sind zudem nicht monoton:
OPENCV ist bei 5~FPS in Low besser als in QHD, PINHOLE erreicht dagegen bei
5~FPS seinen hoechsten Wert in QHD. Die Ursache ist daher wahrscheinlich eine
Interaktion aus Aufloesung, Kameramodell, konkreter Bewegung, Maskenabdeckung
und STS-Optimierung.

Der aktuelle Zeit-/Qualitaetsquotient verwendet einen explorativen, gleich
gewichteten Min-Max-Index aus PSNR, SSIM und invertiertem LPIPS und summiert
die vollständige Dauer zwischen Laufstart und Laufende der archivierten
Pipeline-Läufe. SAM3, COLMAP, Rendering und Metrikcontainer sind darin nur
enthalten, wenn sie innerhalb dieses archivierten Laufs lagen. Für die Hauptaussage
sind deshalb die Rohmetriken sowie spaetere Centerline-/GNSS-Geometriemetriken
massgeblich; die Zeit-/Qualitaetsmatrix wird im Effizienz-/Sensitivitaets-
abschnitt oder im Anhang gezeigt. Ein fairerer Aufloesungsvergleich wuerde
alle Renderings und Ground-Truth-Bilder zusaetzlich in eine gemeinsame
Zielaufloesung und auf identische Eval-Views bringen.

\subsubsection*{Maskenrollen und Einzelansichtsstreuung}
Die erodierte \texttt{middle}-Maske wird im Matrixpfad als konservativer
Objektkern verwendet. Sie prüft die Maskenabdeckung und entscheidet, ob eine
Ansicht in den festen Evaluationssplit aufgenommen werden darf. Für die
eigentliche Bildmetrik wird dagegen die weniger beschnittene
\texttt{default}-Maske verwendet; dies ist im JSON-Feld
\texttt{mask\_level=default} nachvollziehbar. Die beiden Rollen dürfen nicht
verwechselt werden: \texttt{middle} ist ein Robustheits-/Coverage-Gate,
\texttt{default} die Aggregationsmaske für PSNR, SSIM und LPIPS. Im SuGaR-
Coarse-Training bleibt \texttt{middle} zusätzlich die konservative Maske für
den Depth-Normal-Pfad.

Die visuellen Review-Panels in \texttt{review\_ideal} und
\texttt{review\_raw} zeigen bewusst dieselbe \texttt{middle}-Stufe wie das
Coverage-Gate, nicht die \texttt{default}-Maske. Das Panel-Skript
\texttt{export\_mask\_review\_samples.py} wird dazu mit
\texttt{--mask-name middle} aufgerufen. Grund ist die Verwendungsrichtung:
\texttt{middle} steuert die Maskenabdeckungsprüfung, die Aufnahme in den
festen Evaluationssplit und die STS-Eingabe. Was die Sichtkontrolle an den
Panelrändern beurteilt (Kantenqualität, Ausrisse, Verluste), ist damit exakt
die Maske, die nachgelagert über Abbruch, Split-Auswahl und STS-Eingabe
entscheidet. Ein Review auf \texttt{default} würde Ränder zeigen, die in der
Produktionsroute nicht als steuerndes Niveau verwendet werden;
\texttt{small} würde einen zu aggressiven Ausschnitt zeigen.

Die neuen Boxplots verwenden die Einzelansichten aus dem Feld
\texttt{per\_frame}. Sie zeigen, ob ein Gesamtmittelwert durch wenige sehr gute
oder sehr schlechte Ansichten verdeckt wird. Das ist besonders relevant bei
2 FPS und niedriger Auflösung, weil die Evaluationsmengen unterschiedlich groß
sind (zwölf Ansichten bei 2 FPS, dreißig bei 5 FPS) und nicht exakt dieselben
physischen Aufnahmezeitpunkte enthalten.

Für SSIM wird zunächst eine vollständige lokale SSIM-Karte berechnet und
anschließend an den gültigen Maskenpixeln aggregiert. Lokale Fenster am
Maskenrand können deshalb weiterhin Kontext außerhalb der Maske enthalten.
LPIPS wird in einem gemeinsamen Objekt-Crop mit neutralisiertem Außenbereich
berechnet; auch hier bleibt eine begrenzte Randkontextwirkung durch die
rezeptiven Felder des Netzes möglich. Diese Details machen die Werte
reproduzierbar, ändern aber nichts an ihrer Einordnung als Bildmetriken der
Gaussian-Repräsentation.

\subsubsection*{Vorlaeufige quantitative Matrixauswertung}
Die neue Auswertung trennt die Modellstufen. Abbildung~\ref{fig:matrix-overview}
zeigt die archivierten STS-Metriken; die SuGaR-Coarse-Werte sind in der
separaten Abbildung~\ref{fig:matrix-combined-overview} dargestellt. Die Zeilen
beschreiben jeweils Laufvariante und Aufloesung. Innerhalb jeder FPS-Gruppe
werden PSNR, SSIM und LPIPS objektmaskiert aggregiert; die SSIM-Fenster und
LPIPS-Rezeptivfelder können am Rand noch lokalen Kontext enthalten. Ein
Average wird nur gebildet, wenn sowohl der 2-FPS- als auch der
5-FPS-Lauf derselben Konfiguration vollstaendig erfolgreich war.

Die aktuelle Grafikserie basiert auf dem vollständigen Repeat-Batch
\texttt{matrix\_repeat\_20260812} mit 30 erfolgreichen Läufen. Davon entfallen
18 STS-Route-A-Läufe auf die drei Kameramodelle SIMPLE-RADIAL, OPENCV und
PINHOLE (je sechs Kombinationen aus FPS und Auflösung); weitere zwölf Läufe
betreffen SuGaR-Coarse mit SIMPLE-RADIAL und OPENCV. Ein Pinhole-
SuGaR-Coarse-Lauf ist in der Datengrundlage nicht vorhanden und wird daher
nicht vorgetäuscht. Ältere unvollständige Matrixläufe bleiben separat als
Entwicklungs- und Fehlernachweis archiviert.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/sts_masked_overview.pdf}
    \caption{Getrennte quantitative STS-7000-Auswertung der vollständigen
    Original-GS-Route A aus den \texttt{sts\_masked.json}-Dateien.}
    \label{fig:matrix-overview}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/sts_masked_per_frame_boxplots.pdf}
    \caption{Streuung der Einzelansichten in den vollständigen STS-Route-A-Läufen.
    SR ist blau, OPENCV orange und PINHOLE grau dargestellt.}
    \label{fig:matrix-sts-boxplots}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/sugar_coarse_masked_overview.pdf}
    \caption{Getrennte quantitative SuGaR-Coarse-9000-Auswertung. Verwendet
    werden ausschließlich die zwölf Dateien
    \texttt{sugar\_coarse\_masked.json}; STS-Baselines werden nicht als
    SuGaR-Metriken eingesetzt.}
    \label{fig:matrix-combined-overview}
\end{figure}

Die beiden Grafiken sind als Screening- und Effizienzdarstellung geeignet, ersetzen aber
keinen geometrischen Genauigkeitsnachweis. Insbesondere duerfen die scheinbar
hohen Low-Aufloesungswerte nicht isoliert als bessere Rekonstruktion gelesen
werden, weil die Auswertung jeweils in der nativen, bereits herunterskalierten
Bilddomäne erfolgt. Fuer die abschliessende Bachelorbewertung sind deshalb
zusammen mit dieser Grafik die Rohmetriken, die Maskenabdeckung sowie
Centerline-/GNSS-Geometriemetriken zu berichten.

Die historische, separat dokumentierte SuGaR-Coarse-Folgematrix ist ebenfalls
abgeschlossen; sie ist von der oben verwendeten vollständigen Repeat-Serie zu
unterscheiden.
Die separate Konfiguration
\texttt{tools/experiment\_matrix\_sugar.tsv} enthält
\texttt{SIMPLE\_RADIAL/sugar\_coarse} und
	exttt{OPENCV/sugar\_coarse}; bei drei Aufloesungen und zwei FPS-Profilen
entstanden zwölf vollständige Läufe. Die bereits ausgewerteten A-Routen wurden
nicht wiederholt. Vor dem Folgeversuch wurde der technische SuGaR-Renderpfad
repariert: Der Containerpfad \texttt{/opt/sugar} wird explizit in den
Python-Importpfad aufgenommen und HWC-Renderergebnisse werden vor dem Speichern
in das erwartete CHW-Tensorlayout ueberfuehrt. Alle zwölf Manifeste der
	exttt{matrix\_sugar\_followup\_12} tragen den Status \texttt{success}; die
Coarse-Meshgewinnung, das Coarse-Rendering, die objektmaskierte Auswertung und
das Postprocessing wurden archiviert. Die \texttt{sugar\_refined\_masked.json}
weist in allen zwölf Fällen jedoch \texttt{no refined.ply exported} aus. Der
Folgelauf ist daher ein abgeschlossener SuGaR-Coarse-Nachweis, aber noch kein
SuGaR-Refined-Vergleich.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/sugar_coarse_masked_per_frame_boxplots.pdf}
    \caption{Streuung der Einzelansichten in den zwölf SuGaR-Coarse-Läufen.
    Die Punkte stammen aus den archivierten \texttt{per\_frame}-Einträgen;
    Boxen zeigen Quartile und Whisker.}
    \label{fig:matrix-sugar-overview}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/sugar_coarse_vs_sts_delta.pdf}
    \caption{Gepaarte Differenz SuGaR-Coarse minus STS bei identischer
    Follow-up-Konfiguration. Die Darstellung ist ein Rendervergleich und kein
    Mesh- oder Geometrienachweis.}
    \label{fig:matrix-sugar-delta}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/matrix_repeat_2026-08-17/metric_vs_runtime.pdf}
    \caption{Explorative Beziehung zwischen Bildmetriken und der archivierten
    Gesamtlaufzeit des archivierten Pipeline-Laufs. SAM3- und COLMAP-Zeit sind
    nur enthalten, wenn sie innerhalb dieses archivierten Laufs lagen.}
    \label{fig:matrix-metric-runtime}
\end{figure}

Die SuGaR-Coarse-Mittelwerte des Repeat-Batches aus 2 und 5 FPS betragen für
SIMPLE-RADIAL in 720p 21,434 dB PSNR, 0,766 SSIM und 0,1529 LPIPS, in QHD
21,586 dB, 0,757 und 0,1404 sowie in Low 21,874 dB, 0,752 und 0,1253. Für
OPENCV ergeben sich in 720p 21,383 dB, 0,761 und 0,1542, in QHD 21,596 dB,
0,754 und 0,1438 sowie in Low 21,836 dB, 0,747 und 0,1264. Diese Werte sind objektmaskierte
Ansichtsmetriken; sie belegen keine geometrische oder geodätische Genauigkeit.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{docs/grafiken/verwendet_verbessert/matrix_sugar_status_table.pdf}
    \caption{Statusübersicht der zwölf abgeschlossenen SuGaR-Coarse-Folgeläufe.}
\end{figure}

Die neue Auswertung liegt unter
\texttt{docs/grafiken/neu\_metriken\_2026-08-12/}. Sie enthält getrennte
    Übersichten für STS und SuGaR-Coarse, Einzelansichts-Boxplots, eine
    gepaarte STS--SuGaR-Coarse-Deltagrafik sowie eine explorative
    Metrik--Laufzeit-Darstellung. Die CSV-Dateien dokumentieren die
    Run-Metadaten und die Einzelansichten; die Modellstufen werden nicht mehr
    in einer einzigen Metrikzeile vermischt.

Die Darstellungsregeln sind bewusst redundant lesbar: Die Kamera steht in der
x-Achsenbeschriftung, daher ist keine zusätzliche SR-/OPENCV-Legende nötig.
Bei den Boxplots liegen die einzelnen \texttt{per\_frame}-Punkte innerhalb
der zugehörigen x-Kategorie und werden nur geringfügig horizontal verteilt,
damit sie sich nicht gegenseitig verdecken. Die y-Achsen der Boxplots werden
aus allen Einzelwerten einschließlich Ausreißern mit Rand berechnet. In der
Laufzeitgrafik ist die x-Achse fest auf 15 bis 40 Minuten begrenzt. Grün und
Lila kodieren STS und SuGaR-Coarse; blasse beziehungsweise kräftige Farbtöne
kodieren 2 beziehungsweise 5 FPS; Kreis, Quadrat und Dreieck kodieren 720p,
QHD und Low. Die Legende liegt oberhalb rechts
außerhalb der eigentlichen Zeichenfläche. Die Statusgrafik wurde aus der PA
entfernt; Statuszahlen bleiben als Tabelle und in den Manifesten erhalten.
...
\textbf{Reproduzierbare Implementierung:} Die Erweiterung liegt als versionsfixierter lokaler SuGaR-Fork unter \texttt{third\_party/SuGaR} vor; der vollständige Ausgangscommit ist zugleich im Dockerfile festgelegt. Während der Entwicklung überlagert die optionale Compose-Datei \texttt{docker-compose.sugar-dev.yml} diesen Quellstand in den bestehenden Containerpfad \texttt{/opt/sugar}; ein wiederholtes Herunterladen oder Neubauen des Images ist damit nicht erforderlich. Der Runner \texttt{run\_masked\_sugar.sh} verwendet jetzt standardmaessig Variante A: direkte Original-STS-GS-Meshgewinnung mit projizierter Diamond-Mesh-Tiefe, ohne SuGaR-Coarse-Optimierung, Refinement oder UV-Textur. Die fruehere vollstaendige Kette bleibt als explizite \texttt{SUGAR\_MESH\_MODE=sugar\_coarse}-Vergleichsroute erhalten. Die geänderten Module wurden kompiliert; der maskierte RGB-Gradient sowie der maskierte Depth-Normal-Pfad wurden mit realen Kamera- und Maskendaten getestet. Ein untexturiertes A-OBJ wird fuer den Centerline-Einstieg aus dem erhaltenen Coarse-PLY erzeugt.
...
\textbf{Kausale SuGaR-Ablation und neuer Produktionsstandard:} Der offizielle SuGaR-Ablauf
...
Geometrie stuetzt. Aufgrund der visuellen Vierfeld-Ablation ist Variante A
deshalb jetzt der Produktionsstandard. Der Schalter
...
optionale Vertex-Projektion an. Diese A-Route ist nun der automatische
Produktionspfad; die bisherige Coarse-Optimierung bleibt unter
\texttt{SUGAR\_MESH\_MODE=sugar\_coarse} fuer Vergleichslaeufe verfuegbar.
...
\texttt{SURFACE\_LEVEL} bleiben dabei konstant. Die Testergebnisse werden im
folgenden Befund dokumentiert; die Zahlen dienen der Stufenlokalisierung und
ersetzen noch keine Ground-Truth-Evaluation.

\textbf{Ergebnis der Vierfeld-Ablation (06.08.2026):} Die vier Coarse-only-
Varianten wurden mit identischen Extraktionsparametern ausgefuehrt:
$200000$ Zielvertices, $5000000$ Surface-Samples, Surface-Level $0{,}3$,
Poisson-Tiefe $10$, Density-Quantil $0{,}1$, aktivierter Projektion auf
Surface-Samples und dem festen \texttt{SURFACE\_SAMPLE\_SEED=42}. A und B
nutzten die private Hochopazitaetskopie der
STS-Gaussians aus
\texttt{data/05\_3dgs/masked\_sugar\_input/library\_i7000\_c9000\_v200000/};
SuGaR-Coarse-Optimierung wurde dabei uebersprungen. C und D nutzten den neu
erzeugten, erfolgreichen c9000-Checkpoint unter dem Tag
\texttt{compare\_c9000\_v2}; auch diese beiden Varianten stoppten vor
Refinement und UV-Baking.

\begin{center}
\small
\begin{tabularx}{\textwidth}{Xlrr}
\textbf{Variante} & \textbf{Eingang/Tiefe} & \textbf{Vertices} & \textbf{Faces}\\
\hline
...
Die visuelle Bewertung des Projektanwenders ist fuer die Auswahl des
Produktionsweges ausschlaggebend: C zeigt gegenueber A mehr feine Details und
deutlich mehr Spitzen. Diese zusaetzliche Detaildichte ist fuer die
Centerline-Anwendung nicht gewuenscht, weil sie die Oberflaeche nicht naeher an
die Ziel-Gaussians bringt, sondern zusaetzliche Ausreisser erzeugt. D wirkt im
Vergleich zu C geglaetteter, ist aber ebenfalls nicht die bevorzugte
Repraesentation. Das urspruengliche Coarse-Mesh mit noch staerker einzeln
sichtbaren Gaussian-Strukturen wirkt eher C-aehnlich, ist jedoch weder mit C
noch mit D identisch. Eine rein visuelle Zuordnung zu C oder D ist deshalb
nicht belastbar; die Eingangs-Gaussian-Wolke und die konkrete Extraktionsroute
muessen getrennt betrachtet werden.

Variante A wird als Produktstandard festgelegt, weil sie die raeumliche
Stuetze der Original-Gaussians am direktesten erhaelt. Die Gaussians sind in
diesem Projekt nicht nur ein Zwischenformat, sondern die gewuenschte
Geometriehypothese. C und D bleiben wissenschaftliche Vergleichsvarianten:
C untersucht die SuGaR-Coarse-Aenderung mit Diamond-Mesh-Tiefe, D dieselbe
Coarse-Aenderung mit Gaussian-Rasterizer-Tiefe. Ein gutes Splat-Rendering
allein ist weiterhin kein metrischer Genauigkeitsnachweis; die A-Auswahl ist
eine begruendete geometrische Produktionsentscheidung und muss fuer die
Bachelorarbeit zusaetzlich ueber Centerline- und GNSS-Referenzfehler bewertet
werden.

A und B liegen in CloudCompare grossraeumig auf derselben Objektgeometrie,
sind aber nicht identisch: Ihre Vertex- und Facezahlen unterscheiden sich.
Eine zusaetzliche Open3D-Pruefung mit jeweils $20000$ gleichmaessig aus den
Meshes gezogenen Punkten ergab fuer A gegen B mittlere gerichtete
Punktabstaende von etwa $1{,}08\,\mathrm{cm}$ beziehungsweise
$0{,}90\,\mathrm{cm}$; die gerichteten 95\%-Quantile lagen bei etwa
$2{,}35\,\mathrm{cm}$ beziehungsweise $1{,}67\,\mathrm{cm}$. Die maximale
gerichtete Abweichung lag in einer Richtung bei etwa $13{,}3\,\mathrm{cm}$,
in der Gegenrichtung bei etwa $3{,}0\,\mathrm{cm}$. Diese Stichprobenwerte
belegen eine starke globale Uebereinstimmung, aber keine bitweise oder
topologische Gleichheit.

Der relevante Unterschied zeigt sich beim Vergleich mit C und D. Gegenueber A
lagen die gerichteten mittleren Stichprobenabstaende bei C bei etwa
$1{,}79\,\mathrm{cm}$ beziehungsweise $3{,}39\,\mathrm{cm}$ und bei D bei etwa
$1{,}96\,\mathrm{cm}$ beziehungsweise $2{,}73\,\mathrm{cm}$. Die 95\%-Quantile
reichten in den Gegenrichtungen bis etwa $14{,}0\,\mathrm{cm}$ bei C und
$12{,}1\,\mathrm{cm}$ bei D. Die SuGaR-Coarse-Optimierung erzeugt damit
keine beliebig weit entfernte Rekonstruktion, veraendert aber die
Oberflaechenstuetzung und die Rand-/Ausreisserstruktur deutlich genug, um die
Mesh-Topologie und sichtbare Artefakte zu beeinflussen.

Die interne Poisson-Diagnostik stuetzt diese Einordnung: A erzeugte vor der
Decimation $87710$ Foreground-Vertices und $173605$ Foreground-Faces, B
$83776/165768$. Beide hatten keine Background-Punkte. C erzeugte
$111619$ Foreground-Vertices und $220818$ Foreground-Faces sowie eine
zusaetzliche Background-Komponente mit $1568$ Vertices und $2911$ Faces. D
erzeugte $93170$ Foreground-Vertices und $184326$ Foreground-Faces; eine
separate Background-Komponente war dort leer. Die Gaussian-Rasterizer-Tiefe
ist damit als relevanter Einflussfaktor bestaetigt, sie erklaert jedoch nicht
allein den Unterschied zwischen A/B und C/D.

Um den Background-Merge als eigene Ursache zu pruefen, wurde C zusaetzlich
mit \texttt{INCLUDE\_BACKGROUND\_MESH=False} wiederholt. Obwohl dabei dieselben
$6$ Background-Samples erkannt, aber nicht rekonstruiert wurden, veraenderte
sich das finale Mesh nur von $79711/156935$ auf $79698/156916$
Vertices/Faces. Der Background-Merge ist damit ein realer, aber in diesem
kontrollierten Lauf kleiner Einfluss und erklaert nicht die wesentliche
Abweichung von C gegenueber A. Die groessere Ursache liegt in der veraenderten
SuGaR-Coarse-Gaussian-Wolke in Verbindung mit der jeweiligen Tiefenroute;
der Schalter bleibt als object-only-Diagnose erhalten; Variante A verwendet
ihn standardmaessig mit der historischen Einstellung \texttt{True}, wobei im
aktuellen Objektlauf keine relevante Background-Komponente anfiel.

Das fruehere Mesh unter
\texttt{data/sugar\_output/library\_i7000\_c9000\_v200000/coarse\_mesh/05\_3dgs/}
war kein A-Original-GS-Mesh. Die historischen Laufprotokolle
\texttt{data/10\_runs/Alurohr\_THWS\_20260804\_191151/run.log} und
\texttt{data/10\_runs/Alurohr\_THWS\_20260804\_195312/run.log} zeigen
explizit \texttt{Use vanilla 3DGS: False} und das Laden des
\texttt{9000.pt}-Coarse-Checkpoints. Es handelte sich somit nicht um A,
sondern um eine SuGaR-Coarse-Extraktion. Ob sie visuell eher C oder D
entspricht, kann aus dem alten Log allein nicht sicher abgelesen werden; der
damalige Extractor hatte jedoch noch keinen separaten Gaussian-Depth-Schalter
und lief mit der damaligen Standardroute. Diese historischen Meshdateien wurden
spaeter durch \texttt{REPLACE=1} unter dem alten Run-Tag entfernt; ihre
Kennzahlen bleiben in den Logs erhalten. Die alten Laeufe erzeugten ebenfalls
eine Background-Komponente, unter anderem $1509/2881$ beziehungsweise
$1824/3557$ Vertices/Faces vor der Decimation. Das erklaert, warum das alte
Mesh Artefakte zeigen konnte, obwohl die A/B-Meshes direkt auf der
STS-Hochopazitaetswolke gut an deren Positionen liegen.

Ohne gesetzten Seed bleibt die Extraktion stochastisch, weil die
Surface-Samples mit zufaelligen Permutationen gezogen werden. Der Extractor
unterstuetzt deshalb jetzt den optionalen Parameter
\texttt{--surface-sample-seed}; die vier dokumentierten Vergleichslaeufe
verwendeten \texttt{SURFACE\_SAMPLE\_SEED=42}. Der Seed kontrolliert die
zufaellige Auswahl der kamerabasierten Surface-Samples, garantiert aber wegen
moeglicher Bibliotheks- und Hardwareunterschiede keine byteidentischen PLYs.
Vergleiche muessen daher weiterhin dieselben Parameter und Eingangsdateien
verwenden; bei hoher wissenschaftlicher Strenge sind mehrere Seeds oder
Wiederholungen zu berichten.
...
Produktionsstandard Variante A fuer das segmentierte Alurohr. Die
SuGaR-Coarse-Route mit $c=9000$ bleibt darin als historische Vergleichsbasis
erhalten.
