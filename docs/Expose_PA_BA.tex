...
\begin{itemize}
    ...
    \item \textbf{Ausfuehrung:} Der Produktionsweg ist die Docker-Pipeline mit
          GPU unter Linux beziehungsweise WSL. Die Windows-11-COLMAP-Laeufe
          ohne CUDA sind getrennte Vergleichslaeufe. Der lokale SuGaR-Fork ist
          als Git-Submodul auf \texttt{MartinRapps/SuGaR@48bbfdd} festgelegt.
          Nach einem frischen Clone ist deshalb
          \texttt{git submodule update --init --recursive} erforderlich. Die
          aktuelle Projektarbeitskopie enthaelt zusaetzlich eine noch nicht in
          einem Fork-Commit festgeschriebene Aenderung in
          \texttt{third\_party/SuGaR/train.py}, welche die c9000-Route erlaubt.
          Vor einem Release muss diese Aenderung in den Fork uebernommen und
          der Parent-Gitlink aktualisiert werden.
    ...
    \item \textbf{Frame- und COLMAP-Standard:} Der aktuelle Default ist
          ...
          Docker-Hub-Repository nicht vorhanden. Das originale
          \texttt{SIMPLE\_RADIAL}-Modell bleibt fuer GCP/UI und SfM-Auswertung
          erhalten; danach erzeugt \texttt{run\_sfm.sh} automatisch eine
          separate ideale \texttt{PINHOLE}-Szene unter
          \texttt{data/04\_sfm/undistorted/}, die wegen der STS-Einschraenkung
          auf undistortierte Kameramodelle an STS uebergeben wird.
...
damit tote Code. \textbf{Maßnahme:} \texttt{pipeline\_lib.sh} als einzigen
geteilten Funktionsbestand einbinden, den Haupteinstieg
\texttt{run\_pipeline.sh} um ein \texttt{-{}-from <step>}-Argument erweitern
(Schritte: \texttt{gcp|sam3|colmap|sts|sugar|postprocess}), die
\texttt{run\_from\_*.sh}-Skripte entfallen damit zugunsten des Replay-Aufrufs
\texttt{run\_pipeline.sh -{}-from colmap|sts|sugar}, und alle interaktiven Helfer
zentralisieren. Erwartete Reduktion der Shell-Codebasis: ca.~40\,\%.
...
\texttt{tesseract-ocr} nur in Container E vorhalten. Den SuGaR-Container auf den
projekteigenen Fork-Stand festlegen (\texttt{SUGAR\_REF=48bbfdd}), sodass auch
Image-Builds die maskenbewussten Modifikationen enthalten und das bisher
\emph{immer} geladene Entwicklungs-Overlay (\texttt{docker-compose.sugar-dev.yml})
nur noch opt-in über eine Umgebungsvariable (\texttt{SUGAR\_USE\_DEV\_FORK=1})
aktiviert wird.
...
Datensätze, Web-UI, Schreibprojekte (\texttt{Ausarbeitung}, \texttt{Vorlage},
\texttt{Themenfindung}) und Experiment-Skripte in einem Repository. Für die
Übergabe und weitere Wartung wird die Pipeline in ein eigenes, sauber
strukturiertes Repository überführt: ein einziger Einstieg
(\texttt{run\_pipeline.sh -{}-from <step>}), geteilte Module unter
\texttt{src/python/}, diagnostische Skripte unter \texttt{tools/}, ein
\texttt{tests/}-Verzeichnis und das Exposé unter \texttt{docs/}. SuGaR wird als
Git-Submodule des projekt-eigenen Forks
(\texttt{MartinRapps/SuGaR} @ \texttt{48bbfdd}) eingebunden; die Web-UI und die
Agent-Memory-Dokumentation werden mit überführt. Datensätze, Web-UI-ferne
Schreibprojekte und Experiment-Skripte verbleiben im archivierten Ursprungsrepo. Der
Umzug erfolgt als \texttt{git init} mit einem sauberen Initial-Commit, der das
Ursprungsrepo referenziert; die bisherige Historie bleibt im archivierten Repo
unverkürzt erhalten.
